...
namespace parser {  
    ...
    bool Parser::parseStatement() {
        ...
        switch(cur_token){
            ...
            case CONTINUE:
            case BREAK:
                m_node = std::make_shared<BranchStmt>(toks[cur_index]);
                next();
                break;
...
bool Parser::parseBinaryLRExpr() {
   if((check(IDEN)&&!checkh(LBRACE))||isLiteral(cur_token)||
...
bool Parser::parseTupleAccessExpr() {
    if(!parseLiteral()){
        return false;
    }
    if(check(LBRACK)){
...
bool Parser::parseBinaryExpr(int prev_prece) {
    ...
    while(BineryOP(cur_token)) {
        ...
        next();
        m_node = nullptr;
        if(parseBinaryExpr(m_preced + 1)){
            right = m_node;
        }
        left = std::make_shared<BineryExpr>(tok, left, opr, right);
        m_preced = 0;
    }
    m_node = left;
            dump2(__func__);
    ...
    bool Parser::parseBlockStatement() {
        ...
        while(!check(RBRACE)&&cur_token != FEOF) {
            if(!parseStatement()){
                return false;
            }
            statms.push_back(m_node);
    ...
    bool Parser::parseArrayType() {
        ...
        m_node = std::make_shared<ArrayType>(tok, siz, type);
                dump2(__func__);
    ...
    bool Parser::parseStructExpr() {  
        ...
        next();
        m_node = std::make_shared<StructStmt>(tok,sig,e,isDecl);
                dump2(__func__);
...
// unary expression = expression
    ...
    bool Parser::parseEnumFieldExpr() {
        if(!check(IDEN)){
            err::out("expected identifier found ->",toks[cur_index]);
            return false;
        }
    ...
    bool Parser::parseEnum() {
        ...
        next();
        m_node = std::make_shared<EnumExpr>(tok,iden,vals);
        return true;
    }
    ...
    bool Parser::parseStruct() {
        ...
        next();
        m_node = std::make_shared<StructStmt>(tok, sig, element,isDecl);
                dump2(__func__);
    ...
    bool Parser::parseReturn() {
        ...
        next();
        m_node = std::make_shared<ReturnStmt>(tok, val);
                dump2(__func__);
    ...
    bool Parser::parseLetStatm() {
        ...
        m_node = std::make_shared<VarStmt>(tok, v, type, val);
                dump2(__func__);
    ...
    bool Parser::parseConst() {
        ...
        m_node = std::make_shared<VarStmt>(tok,name,type,val);
                dump2(__func__);
